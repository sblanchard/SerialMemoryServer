...
/// Supports OpenAI, Ollama, HTTP service, or pattern-based extraction.
...
public static class EntityExtractionServiceFactory
{
    ...
    /// Priority: OpenAI (if API key) > Ollama (if configured) > HTTP service > Software pattern-based
    /// </summary>
    /// <param name="openAiApiKey">OpenAI API key for GPT-based extraction</param>
    /// <param name="openAiEntityModel">OpenAI model for extraction (defaults to gpt-5-nano-2025-08-07)</param>
    ...
    public static IEntityExtractionService Create(
        ...
        string? openAiEntityModel = null,
        string? ollamaUrl = null,
        string? ollamaModel = null,
        string? httpServiceUrl = null,
        bool useSoftwareAware = true)
    {
        // Prefer OpenAI if API key is provided
        if (!string.IsNullOrWhiteSpace(openAiApiKey))
        {
            return new OpenAiEntityExtractionService(
                apiKey: openAiApiKey,
                model: string.IsNullOrWhiteSpace(openAiEntityModel) ? null : openAiEntityModel);
        }

        // Fall back to Ollama if URL is provided
