...
public class TenantIsolationTests : IAsyncLifetime
{
    ...
    {
        // Arrange: Create memory as Tenant A
    ...
    {
        // Arrange: Create memory as Tenant A
        await using var connSetup = new NpgsqlConnection(_connectionString);
        ...
        await using var connNoContext = new NpgsqlConnection(_connectionString);
        await connNoContext.OpenAsync();
        // Deliberately NOT calling set_tenant_context

        // Assert: The guardrail should cause this to fail with an exception
        var exception = await Assert.ThrowsAnyAsync<Exception>(async () =>
        {
            await connNoContext.ExecuteScalarAsync<int>("SELECT COUNT(*) FROM memories");
        });

        // The exception should mention tenant_id not being set
        Assert.Contains("tenant_id", exception.Message.ToLower());
    }

    [Fact]
    public async Task WriteWithMismatchedTenantId_IsRejected()
    {
        // Arrange: Set context for Tenant A
        await using var conn = new NpgsqlConnection(_connectionString);
        await conn.OpenAsync();
    ...
    public async Task TenantConnectionFactory_SetsTenantContext()
    {
        ...
        var factory = new TenantDbConnectionFactory(_connectionString, tenantContext);
    ...
    {
        // Arrange: Create memories for both tenants
        await using var connA = new NpgsqlConnection(_connectionString);
        await connA.OpenAsync();
        await connA.ExecuteAsync("SELECT set_tenant_context(@TenantId)", new { TenantId = TenantA });
    ...
    {
        // Arrange: Create store for Tenant A
        var tenantContextA = new TestTenantContext(TenantA.ToString());
        var storeA = new PostgresKnowledgeGraphStore(_connectionString, tenantContextA);
        ...
        // Arrange: Create store for Tenant B
        var tenantContextB = new TestTenantContext(TenantB.ToString());
        var storeB = new PostgresKnowledgeGraphStore(_connectionString, tenantContextB);
