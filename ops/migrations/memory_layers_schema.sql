-- =============================================================================
-- Memory Layers Schema (L0-L4 Classification Pipeline)
-- =============================================================================
-- This migration adds the tables needed for the memory classification worker
-- to process memories through L0 (raw) -> L4 (heuristic) layers.
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS knowledge_graph_nodes (
    ...
    node_type VARCHAR(50) NOT NULL,     -- 'fact', 'entity', 'relationship', 'rule'
...
CREATE INDEX IF NOT EXISTS idx_kg_nodes_embedding ON knowledge_graph_nodes USING ivfflat (embedding vector_cosine_ops)
    WITH (lists = 100);
...
-- =============================================================================
-- STEP 7: Add classification columns to memories table
-- =============================================================================
...
-- =============================================================================
-- STEP 9: Create RLS policies with internal admin bypass
-- =============================================================================
...
-- =============================================================================
-- STEP 10: Helper functions for classification
-- =============================================================================
...
BEGIN
    ...
    SET current_layer = p_layer,
        classification_status = CASE WHEN p_layer = 'L4_HEURISTIC' THEN 'COMPLETE' ELSE 'PROCESSING' END,
    ...
    SET current_stage = p_layer,
        status = CASE WHEN p_layer = 'L4_HEURISTIC' THEN 'COMPLETE' ELSE 'PROCESSING' END,
...
DO $$
BEGIN
    RAISE NOTICE 'Memory layers schema migration completed successfully!';
    RAISE NOTICE 'Tables created: memory_layers, memory_processing_queue, knowledge_graph_nodes, memory_classification_events';
